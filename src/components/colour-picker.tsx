...
export function ColourPicker({
  ...
  const isPresetValue =
    hasValue &&
    PRESET_COLOURS.some((c) => c.toLowerCase() === value.toLowerCase())
  const [advancedOpen, setAdvancedOpen] = useState(
    !collapsible || (hasValue && !isPresetValue)
  )
  ...
  const handleTouchMove = useCallback(
    (e: React.TouchEvent, type: 'sv' | 'hue') => {
      e.preventDefault()
  ...
  const isSelected = (preset: string) => {
    ...
    return (
      preset.toLowerCase() === hsvToHex(hsv[0], hsv[1], hsv[2]).toLowerCase()
    )
  }
