...
export function EntityBoardColumn<TObject extends EntityObject>({
  ...
  const totalCount = rows
    ? rows.reduce((sum, r) => sum + r.objects.length, 0)
    : objects.length
  ...
  const childReorderRef = useRef<{ parentId: string; rank: number } | null>(
    null
  )
  ...
  const handleDragOver = useCallback(
    (e: React.DragEvent) => {
      ...
      for (const t of e.dataTransfer.types) {
        if (t.startsWith('application/x-mochi-class-'))
          draggedClass = t.slice(26)
        else if (t.startsWith('application/x-mochi-id-'))
          draggedId = t.slice(23)
      }
      ...
      if (draggedId && !dragSourceRef.current) {
        ...
        if (draggedObjData) {
          ...
          const sourceRow = rowField
            ? draggedObjData.values[rowField] || ''
            : ''
          // Measure card height from the DOM
          const cardEl =
            columnRef.current?.querySelector(
              `[data-card-id="${CSS.escape(draggedId)}"]`
            ) ||
      ...
      if (targetEl) {
        ...
        if (
          !cursorOnGap &&
          distFromTop > edgeZone &&
          distFromBottom > edgeZone
        ) {
          ...
          if (parentCardEl) {
            ...
            if (
              isAlreadySibling ||
              (parentObj &&
                canParent(parentObj.class) &&
                (!draggedId || !isAncestor(parentId, draggedId)))
            ) {
              const sibContainer = targetEl.parentElement!
              const siblings = sibContainer.querySelectorAll(
                ':scope > [data-card-id]'
              )
              ...
              for (let i = 0; i < siblings.length; i++) {
                if (siblings[i].getAttribute('data-card-id') === draggedId)
                  continue
      ...
      if (dropOnEl && dropOnId) {
        ...
        dropModeRef.current = 'between'
        dropTargetCardRef.current = ''
        childReorderRef.current = {
          parentId: siblingReorder.parentId,
          rank: siblingReorder.rank,
        }
  ...
  const handleDrop = useCallback(
    (e: React.DragEvent) => {
      ...
      const dropOnCardId =
        dropModeRef.current === 'on' ? dropTargetCardRef.current : undefined
  ...
  const renderCard = (object: TObject) => (
    <div
      ...
      className='rounded-lg transition-shadow data-drop-target:ring-2 data-drop-target:ring-primary'
    >
      <EntityBoardCard
        ...
        onDoubleClick={
          onCardDoubleClick ? () => onCardDoubleClick(object) : undefined
        }
  ...
  const renderCardsWithGap = (cardObjects: TObject[], rowId?: string) => {
    ...
    if (sameColumn) {
      ...
      cards.splice(
        preview.targetIndex,
        0,
        renderGap(preview.cardHeight, preview.draggedId)
      )
      return cards
    } else {
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align='end'>
                ...
                {totalCount === 0 && (onDeleteColumn || preview) && (
                  <DropdownMenuItem
                    onClick={
                      preview ? undefined : () => setShowDeleteDialog(true)
                    }
                  >
          ...
          >
            <ResponsiveDialogHeader>
              ...
              <ResponsiveDialogDescription className='sr-only'>
                <Trans>Rename this board column</Trans>
              </ResponsiveDialogDescription>
            </ResponsiveDialogHeader>
      ...
      {rows ? (
        rows.map((row, index) => (
          <div
            ...
            onDoubleClick={
              onCreateInRow && !preview
                ? (e) => {
                    if (
                      e.target === e.currentTarget ||
                      (e.target as HTMLElement).closest('[data-card-id]') ===
                        null
                    ) {
          ...
          onDoubleClick={
            preview
              ...
              : (e) => {
                  if (
                    e.target === e.currentTarget ||
                    (e.target as HTMLElement).closest('[data-card-id]') === null
                  ) {
                    onCreateClick?.()
                  }
                }
