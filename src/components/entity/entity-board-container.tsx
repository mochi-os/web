...
export function EntityBoardContainer<TObject extends EntityObject>({
  ...
  const classFields = useMemo(
    () => (defaultClass ? design.fields[defaultClass.id] || [] : []),
    [defaultClass, design.fields]
  )
  const classOptions = useMemo(
    () => (defaultClass ? design.options[defaultClass.id] || {} : {}),
    [defaultClass, design.options]
  )
  ...
  const visibleFields = useMemo(() => {
    ...
    return viewFieldsList
      .map((id) => fieldMap.get(id))
      .filter(Boolean) as typeof classFields
  ...
  const childrenByParent = useMemo(() => {
    ...
    for (const key of Object.keys(map)) {
      map[key] = sortEntityObjects(map[key], {
        field: 'rank',
        direction: 'asc',
      })
    }
  ...
  // Local reorder state
  const [reorderedColumns, setReorderedColumns] =
    useState<EntityFieldOption[]>(statusOptions)
  ...
  const objectsByRowAndStatus = useMemo(() => {
    ...
    objects.forEach((obj) => {
      ...
      const targetStatus =
        grouped[targetRow][status] !== undefined ? status : ''
  ...
  const applyPreviewToList = useCallback(
    (list: TObject[]): TObject[] => {
      if (!dragPreview || dragPreview.mode === 'on') return list
      if (dragPreview.childReorder)
  ...
  const handleDragPreview = useCallback(
    (preview: EntityDragPreview | null) => {
      if (!preview) {
  ...
  const previewSafetyRef = useRef<ReturnType<typeof setTimeout> | undefined>(
    undefined
  )
  useEffect(() => {
  ...
  const countDescendantsInStatus = useCallback(
    (parentId: string, statusValue: string): number => {
      ...
      ? (
          ...
          if (reorderParentId && reorderRank !== undefined) {
            if (draggedObj.parent === reorderParentId) {
              ...
              if (!allowedParents || !allowedParents.includes(parentObj.class))
                return
          ...
          if (dropOnCardId && onReparentObject) {
            ...
            if (!targetObj) return
            const allowedParents = design.hierarchy[draggedObj.class]
            if (!allowedParents || !allowedParents.includes(targetObj.class))
              return
          ...
          if (draggedObj.parent && objectMap[draggedObj.parent]) {
            ...
            const parentRow = rowField
              ? parent?.values[rowField] || ''
              : undefined
          ...
          if (newRank) {
            const topLevel = (objectsByStatus[columnId] || []).filter(
              (o) => o.id !== objectId
            )
  ...
  useEffect(() => {
    ...
    const onDragEnd = (e?: DragEvent) => {
      ...
      if (!e || !e.dataTransfer || e.dataTransfer.dropEffect === 'none') {
        ...
        clearTimeout(previewSafetyRef.current)
        previewSafetyRef.current = setTimeout(
          () => handleDragPreview(null),
          800
        )
      }
  ...
  const renderColumn = (
    ...
    return (
      <div
        ...
        onDragStart={
          isReordering ? (e) => handleColumnDragStart(e, status.id) : undefined
        }
        onDragOver={
          isReordering ? (e) => handleColumnDragOver(e, status.id) : undefined
        }
      ...
      >
        <EntityBoardColumn
          ...
          onCreateClick={
            isReordering || !onCreateClick
              ? undefined
              : () => onCreateClick(status.id)
          }
      ...
      >
        ...
        {columnsToRender.map((status, c) =>
          renderColumn(
            ...
            onCreateClick
              ? (rowId: string) => onCreateClick(status.id, rowId)
              : undefined
          )
        )}
          ...
          >
            <EntityBoardColumn
              ...
              rows={swimlaneRows.map((row) => ({
                ...
                label: row.label,
                colour: row.colour,
                objects: applyPreviewToList(
                  objectsByRowAndStatus[row.id]?.[''] || []
                ),
              }))}
            />
          </div>
  ...
  return (
    <div
      ref={boardRef}
      className='flex gap-4 pt-3 pb-2'
