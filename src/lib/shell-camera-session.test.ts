...
describe('cameraOpen in the shell', () => {
  ...
  it('opens over the protocol and delivers frames until end', async () => {
    ...
    const ends: string[] = []
    const promise = cameraOpen({
      frame: (f) => frames.push(f),
      end: (r) => ends.push(r),
    })
    const start = parentPostMessage.mock.calls.find(
      (c) => c[0]?.type === 'camera.start'
    )?.[0]
    ...
    expect(frames).toHaveLength(2)
    expect(
      (late as unknown as { close: ReturnType<typeof vi.fn> }).close
    ).toHaveBeenCalled()
  })
  ...
  it('stop posts camera.stop and drops later frames', async () => {
    ...
    session.stop()
    expect(
      parentPostMessage.mock.calls.some((c) => c[0]?.type === 'camera.stop')
    ).toBe(true)
    const late = bitmap()
    fromParent({
      type: 'camera.frame',
  ...
  it('tells the shell to stop when the deadline passes unanswered', async () => {
    ...
    try {
      ...
      const stop = parentPostMessage.mock.calls.find(
        (c) => c[0]?.type === 'camera.stop'
      )?.[0]
...
describe('cameraOpen outside the shell', () => {
  it('opens the camera directly and pumps frames', async () => {
    ...
    const frames: ImageBitmap[] = []
    const { session, opened } = await cameraOpen({
      frame: (f) => frames.push(f),
    })
    expect(opened).toEqual({
      ok: true,
